            ...
            for (let parsingRule of parsingRules) {
                for (let separator of parsingRule.separators) {
                    ...
                    if ((index = word.indexOf(separator)) >= 0) {
                        let divided = this.parseWord(word.substring(0, index), errorLimit);
                        if (typeof (divided) != 'undefined') {
                            result.push(...divided);
                        }
                        divided = this.parseWord(word.substring(index + 1), errorLimit);
                        if (typeof (divided) != 'undefined') {
                            result.push(...divided);
                        }
                    }
    ...
    parseString(string, errorLimit) {
        ...
        for (let word of words) {
            ...
            if (Number.isInteger(t) && word != '') {
                ...
                if ((nums = this.parseWord(word, errorLimit)).length > 0) {
                    isNum = false;
                }
            }
            if (nums.length > 0) {
                ...
                if (multiplyed && _multiply_level <= 1) {
                    ...
                    if (_rank < this.#rank && this.#rank != Number.MAX_SAFE_INTEGER) {
                        sum = _sum;
                        rank = _rank;
                    } else {
                        sum = 0;
                        rank = Number.MAX_SAFE_INTEGER;
                        if (this.#rank < Number.MAX_SAFE_INTEGER) {
