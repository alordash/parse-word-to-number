      ...
      if (nums.length > 0) {
         ...
         if (multiplyed && _multiply_level <= 1) {
            if (_rank < Rank) {
               ...
               result.push({ text: Sum, usedWords });
               usedWords = [];
               Sum = _sum;
            }
            Rank = _rank;
            ...
            if (_multiply_level > 1 && multiply_level != _multiply_level) {
               ...
               if (_rank < Rank) {
                  ...
                  result.push({ text: Sum + sum, usedWords });
                  usedWords = [i];
                  sum = _sum;
                  rank = _rank;
                  Sum = 0;
            ...
            if (numLength(sum) < numLength(Sum)) {
               ...
               if (Sum > 0) {
                  if(divIndex >= 0) {
                     ...
                     leftWords = usedWords.splice(divIndex, usedWords.length - index);
                  }
                  result.push({ text: Sum, usedWords });
                  usedWords = [];
               }
               if (sum > 0) {
                  ...
                  result.push({ text: sum, leftWords });
                  usedWords = [i];
               }
            }
         }
         result.push({ text: word, usedWords });
         usedWords = [];
         prevIsNum = false;
         Sum = 0;
         Rank = Number.MAX_SAFE_INTEGER;
