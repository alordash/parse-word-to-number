 ...
 * @returns {Array.<{text: String, value: Number, multipliable: Boolean, limit: Number, rank: Number}>} array of expressions
 */
function LoadExpressionsFile(fileName) {
    ...
    for (let i in arr) {
        let val = arr[i].split(';');
        arr[i] = {};
        [arr[i].text, arr[i].value, arr[i].multipliable, arr[i].limit, arr[i].rank, arr[i].separators] = [val[0], +val[1], +val[2] > 0, +val[3], +val[4], val[5].split('')];
    }
    return arr;
}
